...
.oh-page{min-height:100dvh;background:var(--oh-cream);color:var(--oh-ink);font-family:'DM Mono',monospace;overflow-x:hidden;}
...
.oh-brand{font-family:'Unbounded',sans-serif;font-weight:900;font-size:.95rem;letter-spacing:-.02em;color:var(--oh-ink);white-space:nowrap;}
.oh-brand em{font-style:normal;color:var(--oh-purple);}
.oh-back{display:inline-flex;align-items:center;gap:.45rem;border:0;background:linear-gradient(135deg,var(--oh-start),var(--oh-purple),var(--oh-end));color:#fff;border-radius:999px;padding:.78rem 1.2rem;font-family:'Unbounded',sans-serif;font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em;box-shadow:0 14px 32px rgba(95,7,155,.42);}
...
.oh-title{font-family:'Archivo Black',sans-serif;font-size:clamp(3rem,7vw,6.6rem);line-height:.9;letter-spacing:-.045em;text-transform:uppercase;color:#fff;text-shadow:0 8px 30px rgba(25,0,48,.28);}
...
.oh-user-name{font-family:'Unbounded',sans-serif;font-size:1rem;font-weight:900;color:#fff;line-height:1.2;}
...
.oh-section-title{font-family:'Unbounded',sans-serif;font-size:clamp(1.35rem,2.4vw,2rem);font-weight:900;letter-spacing:-.03em;color:var(--oh-ink);}
.oh-count{color:var(--oh-purple);font-size:.62rem;letter-spacing:.16em;text-transform:uppercase;margin-top:.35rem;}
.oh-refresh{display:inline-flex;align-items:center;gap:.55rem;border:1.5px solid rgba(95,7,155,.16);border-radius:999px;background:#fff;color:var(--oh-purple);padding:.8rem 1rem;font-family:'Unbounded',sans-serif;font-size:.65rem;font-weight:900;text-transform:uppercase;box-shadow:0 8px 22px rgba(31,17,11,.04);}
...
.oh-status{display:inline-flex;align-items:center;gap:.38rem;border-radius:999px;padding:.38rem .68rem;font-family:'Unbounded',sans-serif;font-size:.58rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap;}
...
.oh-item-price{font-family:'Unbounded',sans-serif;font-weight:900;color:var(--oh-purple);font-size:.88rem;}
...
.oh-total{font-family:'Unbounded',sans-serif;font-size:1.55rem;font-weight:900;color:var(--oh-green);text-shadow:0 0 18px rgba(206,246,33,.18);}
...
.oh-empty h2{font-family:'Unbounded',sans-serif;font-size:1.15rem;font-weight:900;color:var(--oh-ink);}
.oh-empty p{max-width:40ch;color:rgba(31,17,11,.56);font-size:.86rem;line-height:1.6;}
.oh-load-more{margin:1.4rem auto 0;display:flex;align-items:center;justify-content:center;gap:.55rem;border:0;border-radius:999px;background:linear-gradient(135deg,var(--oh-start),var(--oh-purple),var(--oh-end));color:#fff;padding:.95rem 1.4rem;font-family:'Unbounded',sans-serif;font-size:.72rem;font-weight:900;text-transform:uppercase;box-shadow:0 14px 36px rgba(95,7,155,.28);}
...
function OrderCard({ order }: { order: OrderRow }) {
  ...
  const customer = order.customer_name || order.delivery_address?.customerName || 'Customer'
    ...
    <article className="oh-card">
      ...
      <div className="oh-card-body">
        <div className="oh-card-head">
          <div>
            <div className="oh-id">Order #{order.id.slice(0, 8).toUpperCase()}</div>
        ...
        <div className="oh-items">
          {order.items.map((item, index) => (
            <div className="oh-item" key={`${order.id}-${item.productId}-${index}`}>
              <div>
              ...
              <div className="oh-item-price">₹{item.pricePerKg * item.quantity}</div>
            </div>
          ))}
        </div>
        ...
        <div className="oh-meta-grid">
          <div className="oh-meta">
            <div className="oh-meta-label">{paymentIcon(order.payment_status)} Payment</div>
            <div className="oh-meta-value">{paymentLabel(order.payment_status)}</div>
          </div>
          <div className="oh-meta">
            <div className="oh-meta-label"><ReceiptText size={14} /> Items</div>
            <div className="oh-meta-value">{order.items.length} cuts · {itemCount} pcs</div>
          </div>
          {order.eta_minutes !== null && (
            <div className="oh-meta">
              <div className="oh-meta-label"><Clock3 size={14} /> ETA</div>
              <div className="oh-meta-value">{order.eta_minutes} minutes</div>
            </div>
          )}
          {phone && (
            <div className="oh-meta">
              <div className="oh-meta-label"><Phone size={14} /> Phone</div>
        ...
        <div className="oh-address">
          <div className="oh-meta-label"><MapPin size={14} /> Delivery Address</div>
          <div className="oh-meta-value">{formatAddress(order)}</div>
        </div>

        {order.driver_name && (
          <div className="oh-address">
            <div className="oh-meta-label"><Truck size={14} /> Delivery Partner</div>
            <div className="oh-meta-value">
              {order.driver_name}{order.driver_phone ? ` · +91 ${order.driver_phone}` : ''}
            </div>
          </div>
        )}
        ...
        {order.notes && (
          <div className="oh-address">
            <div className="oh-meta-label"><ShieldCheck size={14} /> Notes</div>
...
function EmptyState({ tab }: { tab: Tab }) {
  return (
    <div className="oh-empty">
      ...
      <div>
        <h2>{tab === 'current' ? 'No current orders' : 'No order history yet'}</h2>
        <p>
          {tab === 'current'
...
export default function OrderHistoryClient({ user }: { user: AuthUser }) {
  ...
  const fetchHistory = useCallback(async (reset = true) => {
    ...
    try {
      const res = await fetch(`/api/orders/history?offset=${offset}&limit=${PAGE_SIZE}`, { cache: 'no-store' })
      if (res.status === 401) {
        window.location.href = '/'
        return
      }
  ...
  const countText = useMemo(() => {
    const count = activeTab === 'current' ? activeOrders.length : historyOrders.length
    ...
    <div className="oh-page">
      ...
      <header className="oh-hero">
        <div className="oh-nav">
          <Link href="/" className="oh-brand">B&apos;<em>LURU</em> FRESH</Link>
        ...
        <div className="oh-hero-inner">
          <div>
            ...
            <p className="oh-sub">
              Track current orders, review past deliveries, and keep every fresh cut receipt in one polished view.
            </p>
          </div>

          <div className="oh-user-card">
            <div className="oh-user-label">Signed in as</div>
            <div className="oh-user-name">{user.name || 'B’LURU Fresh Customer'}</div>
      ...
      <main className="oh-main">
        <div className="oh-toolbar">
          ...
          <button className="oh-refresh" onClick={refresh} disabled={refreshing}>
