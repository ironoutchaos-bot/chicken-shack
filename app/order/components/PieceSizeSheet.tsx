...
type Props = { open: boolean; productName: string; options: PieceSize[]; onClose: () => void; onConfirm: (size: PieceSize, note: string) => void }
...
export default function PieceSizeSheet({ open, productName, options, onClose, onConfirm }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    setSelected(options.includes('Medium') ? 'Medium' : options[0] ?? 'Medium')
    setNote('')
  }, [open, productName])
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className="piece-size-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="piece-size-sheet" role="dialog" aria-modal="true" aria-labelledby="piece-size-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label className="piece-size-note-label" htmlFor="piece-size-note">Add a note <span>(optional)</span></label>
        <textarea
          id="piece-size-note"
          ...
          rows={2}
        />
        <button type="button" className="piece-size-confirm" onClick={() => onConfirm(selected, note.trim())}>Add to Cart</button>
        <style jsx global>{`
          ...
          .piece-size-note-label{display:block;margin:14px 0 6px;color:#1f110b;font-size:12px;font-weight:800}.piece-size-note-label span{color:#8a7970;font-weight:600}
          .piece-size-note{display:block;width:100%;min-height:62px;resize:none;border:1.5px solid #dcd4ce;border-radius:10px;padding:10px 12px;background:#fff;color:#1f110b;font:500 14px/1.4 inherit;outline:none}.piece-size-note:focus{border-color:#60079d;box-shadow:0 0 0 3px rgba(96,7,157,.08)}.piece-size-note::placeholder{color:#a69992}
