      ...
      <div className="relative flex max-h-[96vh] w-full flex-col rounded-t-[30px] bg-white shadow-2xl sm:mb-5 sm:max-w-[560px] sm:rounded-[28px]">
          ...
          {step === 'choice' && (
            <section className="space-y-5 pb-2">
              ...
              <div className="space-y-3 pt-1">
                <button type="button" onClick={openAwayFromLocation} className="min-h-16 w-full rounded-2xl bg-[#6510a8] px-5 py-4 text-[16px] font-black text-white shadow-sm">Away from my location</button>
        ...
        {step !== 'choice' && (
          <footer className="shrink-0 space-y-2.5 border-t border-stone-100 px-5 pb-[max(env(safe-area-inset-bottom),1rem)] pt-3">
            ...
            {step === 'map' ? <button type="button" onClick={() => setStep('details')} disabled={!canConfirmPin} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#6510a8] py-4 text-[15px] font-black text-white shadow-sm disabled:bg-stone-300 disabled:shadow-none"><CheckCircle2 size={18} />Confirm this location</button> : <button type="button" onClick={handleConfirm} disabled={!canProceed || validating} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-stone-950 py-4 text-[15px] font-black text-white shadow-sm disabled:bg-stone-300 disabled:shadow-none">{validating ? <><Loader2 size={18} className="animate-spin" />Checking delivery area...</> : <><CheckCircle2 size={18} />Confirm address &amp; proceed to pay</>}</button>}
          </footer>
        )}
      </div>
    </div>
